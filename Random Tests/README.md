## ⚠️ Warning: You have entered my experimental graveyard.
(i know. too many READMEs. bear with me)

This folder contains chaotic (and possibly missing files and code) from the model development phase.
The scripts are unoptimized, inconsistent and terrible.

I posted them them just to show my chaotic thought process, which goes back and forth.

If you try to run any of the test files, these will happen:
* missing code - so it might not even work
* if it works:
  - exhaust your RAM
  - crash your runtime
  - run out of PATIENCE

Run only:
* sufficient compute (GPU recommended)
* if you have too much free time and are curious(too much curiosity kills the cat btw)
* if you have the patience of an enlightened being

(run at your own risk. and maybe keep your task manager open)

ファイトだよ！

![hmmm](https://i.pinimg.com/736x/5a/ed/7b/5aed7b32eef240adab815af1f392f697.jpg)

